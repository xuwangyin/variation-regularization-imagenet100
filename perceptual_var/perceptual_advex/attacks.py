...
class LpAttack(nn.Module):
    def __init__(self, model, bound, num_iterations, norm, step=None, **kwargs):
        super().__init__()
        self.model = model
        if step is None:
            self.step = bound / (num_iterations ** 0.5)
        else:
            self.step = step
        self.norm = norm
        ...
        self.bound = bound
    def forward(self, inputs, labels):
        return attack_pgd(self.model, inputs, labels, self.bound, self.step, self.num_iterations, restarts=1, norm=self.norm)
        

class LinfAttack(LpAttack):
    def __init__(self, model, dataset_name, bound=None, num_iterations=10, **kwargs):
        if bound is None:
            bound = {
                'cifar': 8/255,
                'imagenet100': 8/255,
