...
if __name__ == '__main__':
    ...
    parser.add_argument('--var_reg', type=float, default=1.0,
                        help='strength of variation reg')
    ...
    var_reg = eval(args.attack[0].replace('FastLagrangePerceptualAttack', 'FastLagrangePerceptualVariation'))
    validation_attacks = [
        NoAttack(),
        #LagrangePerceptualAttack(model, num_iterations=10, bound=1, lpips_model=lpips_model)
        #PerceptualPGDAttack(model, num_iterations=10, bound=0.5, lpips_model='alexnet', projection='newtons')
    ...
    attacks_part = '-'.join(args.attack + flags) + '_var_{:.2f}'.format(args.var_reg)
    ...
    if torch.cuda.is_available():
        ...
        var_reg = nn.DataParallel(var_reg, device_ids)
    ...
    def run_iter(
        ...
        attack_var1_inputs = inputs.clone()
        attack_var2_inputs = inputs.clone()
        ...
        if args.var_reg > 0:
            logits_var1 = model(attack_var1_inputs)
            logits_var2 = model(attack_var2_inputs)
        ...
        loss = loss.mean()
        if args.var_reg > 0:
            loss = loss + args.var_reg * torch.norm(logits_var1 - logits_var2, p=2, dim=-1).mean()
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        acc = evaluation.evaluate_against_attacks(
        ...
        if acc > best_acc:
            best_acc = acc
            print('UPDATING BEST ACC = {}'.format(best_acc))
            model_best_fname = os.path.join(log_dir, 'model_best.pth')
            torch.save(state, model_best_fname)
