...
def get_dataset_model(
    ...
    **kwargs,
    ...
    if arch.startswith('rob-') or (
    ...
    elif dataset_name.startswith('cifar') and 'vgg' in arch:
        model = CifarVGG16(ReLU())
        ...
        model = TradesWideResNet(depth=28, widen_factor=10)
        if checkpoint_fname is not None:
            state = torch.load(checkpoint_fname)
            model.load_state_dict(state["model"])
    ...
    elif dataset_name.startswith('imagenet') and 'vgg16' in arch:
    ...
    elif 'wrn' in arch or 'vgg' in arch:
